...
class AIService:
    ...
    def _get_openai_models(api_key: str, proxy: str = None) -> List[Dict]:
        ...
        data = AIService._make_request('GET', url, proxy=proxy, headers=headers, timeout=30)
        # 过滤出 GPT 相关模型
        models = []
    ...
    def format_data_for_prompt(
        ...
        if kline_data:
            ...
            prompt_parts.append(f"最近 {len(last_k)} 个交易日数据 (共提供 {len(kline_data)} 天历史):")
            ...
            for k in last_k:
                ...
                prompt_parts.append(f"| {k['date']} | {k['open']} | {k['close']} | {k['high']} | {k['low']} | {change} | {vol_str} | {vol_ratio} |")
            
            # 成交量趋势分析
            recent_5_vol = sum([k['volume'] for k in last_k[-5:]]) / 5 if len(last_k) >= 5 else 0
            recent_10_vol = sum([k['volume'] for k in last_k[-10:]]) / 10 if len(last_k) >= 10 else 0
            
            prompt_parts.append(f"\n**成交量分析:**")
            prompt_parts.append(f"- 30日平均成交量: {AIService._format_volume(int(avg_volume))}")
            prompt_parts.append(f"- 近5日平均成交量: {AIService._format_volume(int(recent_5_vol))}")
            prompt_parts.append(f"- 近10日平均成交量: {AIService._format_volume(int(recent_10_vol))}")
        ...
        if minute_data:
            ...
            for d in sorted(sorted_dates):
                ...
                prompt_parts.append(f"当日总成交量: {AIService._format_volume(total_vol)}")
                ...
                for i, item in enumerate(items):
                    ...
                    if i == 0 or i == len(items)-1 or i % 15 == 0:
                        prompt_parts.append(f"| {item['time']} | {item['price']} | {item.get('avg_price', '-')} | {AIService._format_volume(vol)} | {AIService._format_volume(cumulative_vol)} |")
        
        prompt_parts.append(f"\n### 分析要求\n")
        prompt_parts.append("请重点分析：")
        prompt_parts.append("1. 价量配合关系（放量上涨/缩量下跌等）")
        prompt_parts.append("2. 成交量异动情况")
        prompt_parts.append("3. 短期趋势判断")
        prompt_parts.append("4. 操作建议（买入/持有/卖出）")
        
        return "\n".join(prompt_parts)

    @staticmethod
    def call_llm(provider: str, api_key: str, model: str, prompt: str, proxy: str = None, max_retries: int = 3) -> str:
        """
        ...
        for attempt in range(max_retries):
            try:
                if provider.lower() == "openai" or provider.lower() == "gpt":
                    return AIService._call_openai(api_key, model, system_prompt, prompt, proxy)
                elif provider.lower() == "gemini":
                    return AIService._call_gemini(api_key, model, system_prompt, prompt, proxy)
                elif provider.lower() == "claude":
                    return AIService._call_claude(api_key, model, system_prompt, prompt, proxy)
                else:
                ...
                return f"分析失败: 代理连接失败，请检查代理地址是否正确（当前: {proxy}）"
    ...
    @staticmethod
    def _call_openai(api_key: str, model: str, system_prompt: str, user_prompt: str, proxy: str = None) -> str:
        ...
        res_json = AIService._make_request('POST', url, proxy=proxy, headers=headers, json=data, timeout=90)
        return res_json['choices'][0]['message']['content']

    @staticmethod
    def _call_gemini(api_key: str, model: str, system_prompt: str, user_prompt: str, proxy: str = None) -> str:
        if not model:
            model = "gemini-pro"
    ...
    @staticmethod
    def _call_claude(api_key: str, model: str, system_prompt: str, user_prompt: str, proxy: str = None) -> str:
