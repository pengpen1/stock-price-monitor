...
export const deleteGroupApi = async (group: string, deleteStocks: boolean = false) => {
  const response = await api.delete(`/groups/${encodeURIComponent(group)}`, {
    params: { delete_stocks: deleteStocks },
  })
  return response.data
}
...
// 更新交易记录
export const updateTradeRecord = async (
  recordId: string,
  data: {
    type?: "B" | "S" | "T"
    price?: number
...
export const getStockTradeRecords = async (stockCode: string, limit: number = 100) => {
  const response = await api.get(`/records/trade/${stockCode}`, {
    params: { limit },
  })
  return response.data
}
...
export const getStockAIRecords = async (stockCode: string, limit: number = 50) => {
  const response = await api.get(`/records/ai/${stockCode}`, {
    params: { limit },
  })
  return response.data
}
...
// 获取模拟会话列表
export const getSimulationSessions = async (
  stockCode?: string,
  status?: string,
  limit: number = 50,
) => {
...
export const updateNoteApi = async (filename: string, content: string) => {
  const response = await api.put(`/notes/${encodeURIComponent(filename)}`, {
    content,
  })
  return response.data
}
...
export const renameNoteApi = async (filename: string, newName: string) => {
  const response = await api.put(`/notes/${encodeURIComponent(filename)}/rename`, {
    new_name: newName,
  })
  return response.data
}
