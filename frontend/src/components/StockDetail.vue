...
<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
        ...
        <div class="text-right">
          <div class="mb-1 flex items-center justify-end gap-2">
            ...
            <div class="relative" ref="simMenuRef">
              <button
                @click="showSimMenu = !showSimMenu"
                class="flex items-center gap-1 rounded-md border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-medium whitespace-nowrap text-blue-600 transition-all hover:border-blue-300 hover:bg-blue-100"
              >
                实盘模拟
              ...
              <div
                v-if="showSimMenu"
                class="absolute top-full right-0 z-20 mt-1 min-w-[130px] rounded-lg border border-slate-200 bg-white py-1 shadow-lg"
              >
                <button
                  @click="openSimulationConfig"
                  class="w-full px-3 py-2 text-left text-sm whitespace-nowrap text-blue-600 hover:bg-blue-50"
                >
                  + 新建模拟
                </button>
                <button
                  @click="openSimulationList"
                  class="w-full px-3 py-2 text-left text-sm whitespace-nowrap text-slate-700 hover:bg-slate-50"
                >
                  模拟记录
                </button>
              </div>
            ...
            <div class="relative" ref="recordMenuRef">
              <button
                @click="showRecordMenu = !showRecordMenu"
                class="flex items-center gap-1 rounded-md border border-slate-200 bg-slate-100 px-3 py-1.5 text-xs font-medium whitespace-nowrap text-slate-600 transition-all hover:border-slate-300 hover:bg-slate-200"
              >
                复盘记录
                <span class="text-[10px]">▼</span>
              </button>
            ...
            <button
              @click="openAIModal('fast')"
              class="rounded-md border border-slate-200 bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-600 transition-all hover:border-slate-300 hover:bg-slate-200"
            >
              快速分析
            </button>
            <button
              @click="openAIModal('precise')"
              class="rounded-md border border-slate-200 bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-600 transition-all hover:border-slate-300 hover:bg-slate-200"
            >
              精准分析
            </button>
          </div>
          <div class="text-3xl font-bold" :class="priceClass">
            {{ stockInfo.price || "--" }}
          </div>
    ...
    <TradeRecordList
      v-model:visible="showTradeRecordList"
      :stock-code="code"
      @openJournal="$emit('openJournal')"
    />
    <TradeRecordModal
      v-model:visible="showAddTradeRecord"
      :stock-code="code"
      @saved="onTradeRecordSaved"
    />
    <AIRecordList v-model:visible="showAIRecordList" :stock-code="code" />
    <SimulationConfigModal
      v-model:visible="showSimConfigModal"
      :stock-code="code"
    ...
    />
    <SimulationListModal
      v-model:visible="showSimListModal"
      :stock-code="code"
...
// ========== 计算属性 ==========
const tabs = [{ key: "minute" }, { key: "day" }, { key: "week" }, { key: "month" }]
