  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4">
    <div class="mx-auto max-w-7xl">
      ...
      <div class="grid grid-cols-12 gap-4">
        <!-- 左侧：K线图 -->
        <div class="col-span-8 rounded-xl bg-white p-4 shadow-sm">
          <div class="mb-3 flex items-center justify-between">
            <h3 class="text-sm font-semibold text-slate-700">日K线图</h3>
            <div class="flex items-center gap-2 text-xs text-slate-500">
              <span class="flex items-center gap-1">
                <span class="h-0.5 w-3 bg-orange-500"></span> MA5
              </span>
              <span class="flex items-center gap-1">
                <span class="h-0.5 w-3 bg-blue-500"></span> MA10
              </span>
              <span class="flex items-center gap-1">
                <span class="h-0.5 w-3 bg-purple-500"></span> MA20
              </span>
            </div>
          </div>
          <div style="height: 400px">
            <v-chart v-if="klineOption" :option="klineOption" autoresize class="h-full w-full" />
        ...
        <div class="col-span-4 space-y-4">
          ...
          <div class="rounded-xl bg-white p-4 shadow-sm">
            <h3 class="mb-3 text-sm font-semibold text-slate-700">账户信息</h3>
            <div class="space-y-3">
              <div class="flex justify-between">
                <span class="text-slate-500">可用资金</span>
                <span class="font-medium text-slate-800"
                  >¥{{ formatMoney(session?.current_capital || 0) }}</span
                >
              </div>
              <div class="flex justify-between">
              ...
              <div class="flex justify-between">
                ...
                <span class="font-medium text-slate-800"
                  >¥{{ (session?.cost_price || 0).toFixed(2) }}</span
                >
              </div>
              <div class="flex justify-between">
              ...
              <div class="flex justify-between">
                ...
                <span class="text-lg font-semibold" :class="totalProfitClass">
          ...
          <!-- 当日行情 -->
          <div class="rounded-xl bg-white p-4 shadow-sm">
            <h3 class="mb-3 text-sm font-semibold text-slate-700">当日行情</h3>
            <div class="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <div class="font-medium" :class="dayChangeClass">
            ...
            <button
              @click="showMinuteModal = true"
              class="mt-3 w-full rounded-lg bg-blue-50 px-3 py-2 text-sm text-blue-600 hover:bg-blue-100"
            >
              查看分时图
            </button>
          </div>
          ...
          <!-- 交易操作 -->
          <div class="rounded-xl bg-white p-4 shadow-sm">
            <h3 class="mb-3 text-sm font-semibold text-slate-700">交易操作</h3>

            <!-- 先选择操作类型 -->
            <div class="mb-3">
              <label class="mb-1 block text-xs text-slate-500">操作类型</label>
              <div class="grid grid-cols-3 gap-2">
                <button
                  @click="selectedTradeType = 'buy'"
                  :class="
                    selectedTradeType === 'buy' ? 'bg-red-500 text-white' : 'bg-red-50 text-red-600'
                  "
                ...
                <button
                  @click="selectedTradeType = 'skip'"
                  :class="
                    selectedTradeType === 'skip'
            ...
            <div v-show="selectedTradeType !== 'skip'" class="mb-4">
              <label class="mb-1 block text-xs text-slate-500">操作理由</label>
              <textarea
                v-model="tradeReason"
                rows="2"
                placeholder="请输入操作理由..."
                class="w-full resize-none rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-400 focus:outline-none"
              ></textarea>
            </div>

            <!-- 确认按钮 -->
            <button
              @click="executeTrade(selectedTradeType)"
              :disabled="!canExecuteTrade || trading"
              :class="confirmButtonClass"
              class="w-full rounded-lg px-3 py-2.5 text-sm font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-50"
            >
      ...
      <div class="mt-4 rounded-xl bg-white p-4 shadow-sm">
        <h3 class="mb-3 text-sm font-semibold text-slate-700">交易记录</h3>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr v-for="trade in session.trades" :key="trade.day" class="border-b border-slate-50">
                ...
                <td class="py-2">
                  <span
                    :class="tradeTypeClass(trade.type)"
                ...
                <td class="py-2">
                  {{ trade.type !== "skip" ? "¥" + trade.price.toFixed(2) : "-" }}
                </td>
                <td class="py-2">
                  {{ trade.type !== "skip" ? trade.quantity + "股" : "-" }}
                </td>
                <td class="max-w-xs truncate py-2" :title="trade.reason">
                  {{ trade.reason || "-" }}
                </td>
        ...
        <div v-else class="py-4 text-center text-slate-400">暂无交易记录</div>
      </div>
    </div>

    <!-- 分时图弹窗 -->
    <SimulationMinuteModal
      v-model:visible="showMinuteModal"
      :session-id="sessionId"
      :date="currentDate"
      :stock-name="session?.stock_name || ''"
    />
...
const confirmButtonText = computed(() => {
  switch (selectedTradeType.value) {
    case "buy":
      return `确认买入 ${tradeQuantity.value} 股`
    case "sell":
      return `确认卖出 ${tradeQuantity.value} 股`
    default:
      return "确认跳过"
  }
})
...
const setMaxBuy = () => {
  ...
  const maxShares =
    Math.floor(session.value.current_capital / currentDayData.value.close / 100) * 100
...
const setHalfBuy = () => {
  if (!session.value || !currentDayData.value) return
  const halfShares =
    Math.floor(session.value.current_capital / 2 / currentDayData.value.close / 100) * 100
...
const klineOption = computed(() => {
  ...
  const volumes = klineData.value.map((d) => ({
    value: d.volume,
    itemStyle: {
      color: d.close >= d.open ? "rgba(255,77,79,0.7)" : "rgba(82,196,26,0.7)",
    },
  }))
  ...
  // 交易标记点
  const tradeMarks = (session.value?.trades || [])
    .map((trade) => {
