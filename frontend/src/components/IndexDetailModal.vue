<template>
  ...
  >
    <div
      class="flex max-h-[90vh] w-4/5 max-w-5xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl"
    >
      <!-- 头部 -->
      <div class="flex items-center justify-between border-b border-slate-200 bg-slate-50 p-4">
        <div class="flex items-center gap-4">
          ...
          <span class="text-sm" :class="getPriceClass(indexInfo.change_percent)">
            {{ parseFloat(indexInfo.change_percent || "0") >= 0 ? "+" : ""
            }}{{ indexInfo.change_percent }}%
          </span>
        </div>
        <button
          @click="close"
        ...
        <div v-else class="space-y-4">
          ...
          <div class="grid grid-cols-4 gap-3 text-sm">
            <div class="rounded-lg bg-slate-50 p-3">
              <div class="text-xs text-slate-500">今开</div>
          ...
          <div class="rounded-lg border border-slate-200 bg-white p-4">
            <h4 class="mb-3 text-sm font-semibold text-slate-700">分时走势</h4>
            <div ref="minuteChartRef" class="h-64"></div>
          </div>

          <!-- K线图 -->
          <div class="rounded-lg border border-slate-200 bg-white p-4">
            <h4 class="mb-3 text-sm font-semibold text-slate-700">近期走势（日K）</h4>
...
const renderMinuteChart = () => {
  ...
  // 只取今天的数据
  const today =
    minuteData.value.length > 0 ? minuteData.value[minuteData.value.length - 1].date : ""
