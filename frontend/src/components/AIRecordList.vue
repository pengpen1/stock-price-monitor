<template>
  ...
  >
    <div
      class="bg-gray-900 rounded-xl shadow-2xl w-[750px] max-h-[85vh] flex flex-col overflow-hidden border border-gray-700"
    >
      <!-- 头部 -->
      <div
      ...
      <div class="flex-1 overflow-auto p-4">
        ...
        <div v-else class="space-y-3">
          ...
          >
            ...
            >
              <div
                class="rendered-markdown text-sm text-gray-300 max-h-64 overflow-auto"
...
const bullishCount = computed(
  () => records.value.filter((r) => r.signal === "bullish").length
);
const cautiousCount = computed(
  () => records.value.filter((r) => r.signal === "cautious").length
);
const bearishCount = computed(
  () => records.value.filter((r) => r.signal === "bearish").length
);
