<template>
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        class="relative flex max-h-[90vh] w-[900px] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl"
      >
        <!-- 头部 -->
        <div
        ...
        >
          <div>
            ...
            <p class="text-sm text-slate-500">
              {{ session?.stock_name }} · {{ session?.start_date }} 至
              {{ session?.end_date }}
            </p>
          </div>
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          <div class="mb-6 grid grid-cols-4 gap-4">
            <div class="rounded-xl bg-slate-50 p-4 text-center">
              <div class="mb-1 text-sm text-slate-500">最终资产</div>
              <div class="text-xl font-bold" :class="profitClass">
                ¥{{ formatMoney(result?.final_capital || 0) }}
              </div>
            </div>
            <div class="rounded-xl bg-slate-50 p-4 text-center">
              <div class="mb-1 text-sm text-slate-500">收益率</div>
              <div class="text-xl font-bold" :class="profitClass">
                {{ (result?.profit_rate || 0) >= 0 ? "+" : ""
                }}{{ (result?.profit_rate || 0).toFixed(2) }}%
              </div>
            </div>
            <div class="rounded-xl bg-slate-50 p-4 text-center">
              <div class="mb-1 text-sm text-slate-500">胜率</div>
              <div class="text-xl font-bold text-slate-800">
                {{ (result?.win_rate || 0).toFixed(1) }}%
              </div>
            </div>
            <div class="rounded-xl bg-slate-50 p-4 text-center">
              <div class="mb-1 text-sm text-slate-500">最大回撤</div>
          ...
          <div class="mb-6">
            ...
            <div v-if="analyzing" class="rounded-xl bg-slate-50 p-6 text-center">
              <div
            ...
            <div
              v-else-if="aiResult"
              class="rounded-xl bg-gradient-to-br from-slate-50 to-slate-100 p-5"
            >
              <!-- 评分展示 -->
              <div class="mb-4 flex items-center gap-6">
                <div
                  class="flex h-24 w-24 items-center justify-center rounded-full text-3xl font-bold"
                  :class="gradeColorClass"
                >
                  {{ aiResult.grade }}
                </div>
                <div>
                  <div class="mb-1 text-3xl font-bold text-slate-800">{{ aiResult.score }} 分</div>
              ...
              <div class="mb-4 grid grid-cols-2 gap-4">
                <div>
                  <h5 class="mb-2 text-sm font-medium text-green-600">✓ 优点</h5>
                  <ul class="space-y-1 text-sm text-slate-600">
                    <li v-for="(s, i) in aiResult.strengths" :key="i">• {{ s }}</li>
                  </ul>
                </div>
                <div>
                  <h5 class="mb-2 text-sm font-medium text-red-600">✗ 不足</h5>
                  <ul class="space-y-1 text-sm text-slate-600">
              ...
              <!-- 建议 -->
              <div class="mb-4">
                <h5 class="mb-2 text-sm font-medium text-blue-600">💡 建议</h5>
                <ul class="space-y-1 text-sm text-slate-600">
              ...
              <!-- 详细分析 -->
              <div v-if="aiResult.analysis" class="rounded-lg bg-white p-3">
                <h5 class="mb-2 text-sm font-medium text-slate-700">详细分析</h5>
                <p class="text-sm whitespace-pre-wrap text-slate-600">
                  {{ aiResult.analysis }}
                </p>
              </div>
            </div>
            ...
            <!-- 未获取AI评分时显示 -->
            <div v-else class="rounded-xl bg-slate-50 p-6">
              <div class="mb-4 text-center text-slate-400">点击上方按钮获取 AI 评分分析</div>
              ...
              <div v-if="aiPrompt" class="mt-4">
                ...
                <div
                  class="max-h-40 overflow-y-auto rounded-lg border border-slate-200 bg-white p-3"
                >
                  <pre class="font-mono text-xs whitespace-pre-wrap text-slate-600">{{
                    aiPrompt
                  }}</pre>
                </div>
              </div>
          ...
          <!-- 交易明细 -->
          <div>
            <h4 class="mb-3 text-sm font-semibold text-slate-700">交易明细</h4>
              ...
              <table class="w-full text-sm">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="max-w-xs truncate px-4 py-2" :title="trade.reason">
                      {{ trade.reason || "-" }}
                    </td>
                  </tr>
                </tbody>
        ...
        <!-- 底部 -->
        <div class="flex flex-shrink-0 justify-end border-t border-slate-100 px-6 py-4">
          <button
            @click="close"
            class="rounded-lg bg-blue-500 px-6 py-2.5 text-sm font-medium text-white hover:bg-blue-600"
          >
            完成
          </button>
        </div>
...
const calculateLocalResult = () => {
  ...
  if (props.klineData.length > 0 && session.trades.length > 0) {
    ...
    const simKlines = props.klineData.slice(
      Math.max(0, startIdx - (props.klineData.length - session.total_days)),
    )
