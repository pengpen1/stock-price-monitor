...
<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="flex justify-between items-center mb-5">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20">
        ...
        <div class="flex items-center gap-2">
          <button @click="$emit('openJournal')" class="px-3 py-2.5 text-sm text-slate-600 bg-white border border-slate-200 rounded-xl hover:shadow-sm transition-all duration-200 cursor-pointer" title="交易日志">📝</button>
          <button @click="$emit('openNotes')" class="px-3 py-2.5 text-sm text-slate-600 bg-white border border-slate-200 rounded-xl hover:shadow-sm transition-all duration-200 cursor-pointer" title="笔记">📓</button>
          <button @click="showUserGuide = true" class="px-3 py-2.5 text-sm text-slate-600 bg-white border border-slate-200 rounded-xl hover:shadow-sm transition-all duration-200 cursor-pointer" title="使用手册">📖</button>
          <button @click="showChangelog = true" class="px-3 py-2.5 text-sm text-slate-600 bg-white border border-slate-200 rounded-xl hover:shadow-sm transition-all duration-200 cursor-pointer" title="更新日志">📋</button>
          <button @click="$emit('openSettings')" class="px-3 py-2.5 text-sm text-slate-600 bg-white border border-slate-200 rounded-xl hover:shadow-sm transition-all duration-200 cursor-pointer">⚙️</button>
        </div>
      </div>
      ...
      <!-- 错误提示 -->
      <div v-if="errorMsg" class="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg mb-4 text-sm">
        {{ errorMsg }}
      </div>

      <!-- 预警通知 -->
      <div v-if="alertNotifications.length > 0" class="mb-4 space-y-2">
        <div v-for="(alert, idx) in alertNotifications" :key="idx" class="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-lg text-sm flex justify-between">
          <div>
      ...
      <div class="flex items-center justify-between mb-3 px-1">
        <div class="flex items-center gap-2">
          ...
          <button
            @click="currentGroup = ''"
            :class="!currentGroup ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-sm shadow-blue-500/20' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 hover:border-slate-300'"
          ...
          <button
            ...
            :class="currentGroup === g ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-sm shadow-blue-500/20' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 hover:border-slate-300'"
            class="px-3.5 py-1.5 text-xs rounded-lg transition-all duration-200 font-medium"
          >{{ g }}</button>
          <button @click="showAddGroupModal = true" class="px-3 py-1.5 text-xs text-blue-500 bg-blue-50 border border-blue-100 rounded-lg hover:bg-blue-100 hover:border-blue-200 transition-all duration-200 font-medium">
            + 新建
          </button>
        </div>
        <div class="flex items-center gap-2">
          <span class="text-sm text-slate-500 font-medium">排序:</span>
          <select v-model="sortBy" class="text-xs bg-white border border-slate-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all duration-200 cursor-pointer">
      ...
      <div class="mt-4 h-6 overflow-hidden relative">
        <transition name="quote-fade" mode="out-in">
          <div :key="currentQuoteIndex" class="text-center text-xs text-slate-400 flex items-center justify-center gap-2">
    ...
    <!-- 弹窗组件 -->
    <AlertModal :visible="showAlertModal" :stock="currentAlertStock" :alerts="alerts" @close="showAlertModal = false" @save="saveAlert" />
    <GroupModal :visible="showAddGroupModal" :pending-stock="pendingGroupStock" @close="closeAddGroupModal" @add="addGroup" />
...
// ========== 使用 Composable ==========
const {
  newStockCode, loading, errorMsg, alertNotifications, focusedStock,
  currentGroup, sortBy, groupList, alerts, stockGroups,
  indexList, sortIcon, filteredStocks,
  toggleSort, handleAddStock, handleRemoveStock,
  handleDragStart, handleDrop,
  addGroup, deleteGroup, moveStockToGroup, moveStockToTop, moveStockToBottom,
  saveAlert, dismissAlert, handleSetFocus,
...
// ========== 语录轮播 ==========
const stockQuotes = [
  '一季报披露：4月1日-4月30日', '中报披露：7月1日-8月31日', '三季报披露：10月1日-10月31日', '年报披露：次年1月1日-4月30日',
  '投资有风险，入市需谨慎', '补强不补弱，顺势而为', '不要和趋势作对，趋势是你最好的朋友', '牛市不言顶，熊市不言底',
  '在高潮时警惕，在退潮时理智', '高位横盘不突破，久盘必跌', '宁可错过，不可做错', '量在价先，成交量是股价的先行指标',
  '底部放量要注意，顶部放量要警惕', '截断亏损，让利润奔跑', '空仓也是一种操作', '卖飞光荣，套牢可耻',
  '会买的是徒弟，会卖的是师傅', '不要把所有鸡蛋放在一个篮子里', '知行合一，严格执行交易纪律', '市场永远是对的，错的只是自己',
]
...
<style scoped>
.quote-fade-enter-active, .quote-fade-leave-active { transition: all 0.5s ease; }
