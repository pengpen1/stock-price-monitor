      ...
      series: [
        {
          ...
          yAxisIndex: 0,
          lineStyle: {
            color: prices[prices.length - 1] >= preClose ? "#ff4d4f" : "#52c41a",
            width: 1.5,
          },
          areaStyle: {
            color: {
              ...
              colorStops: [
                {
                  offset: 0,
                  color:
                    prices[prices.length - 1] >= preClose
                      ? "rgba(255,77,79,0.2)"
                      : "rgba(82,196,26,0.2)",
                },
    ...
    return {
      ...
      series: [
        ...
        },
        {
          ...
          tooltip: {
            formatter: (params: any) =>
              `<div style="font-size:12px"><div style="font-weight:bold;margin-bottom:4px">${params.data.typeLabel}</div><div>价格: ¥${params.data.price}</div><div>手数: ${params.data.quantity}手</div><div>时间: ${params.data.tradeTime}</div><div style="margin-top:4px;color:#666">${params.data.reason}</div></div>`,
          },
        },
      ],
    }
