...
- [x] 更新版本，新接口一定要兼容旧数据，不能让用户更新次版本，直接把数据清空了
...
- [x] 用户issues：
  ...
  1）[x] 支持兼容`openai`的自定义`url`

  2）[x] mac系统的用户下载了mac客户端，添加股票说后端没启动（可能是因为后端代码打包后默认是.exe，在mac上无法运行）

  3）[x] mac系统的用户主窗口点了关闭（红色圆点）悬浮窗还在（这是正常现象，Windows用户也是）。是否可以设置成双击悬浮窗唤醒主窗口呢？点击图标唤醒主窗口？（满足用户提的两个需求）
